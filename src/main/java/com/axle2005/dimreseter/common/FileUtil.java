...
public class FileUtil {
	...
	public static void replaceLevelDat(String path) {
		File location = new File(path);
		if (location.isDirectory()) {
			String[] entries = location.list();
			for (String s : entries) {
				File currentFile = new File(location.getPath(), s);
				if (currentFile.getName().equals("level.dat") || currentFile.getName().equals("level.dat_old")) {
					currentFile.delete();
				}

			}
			try {
				Files.copy(new File(DimReseter.getInstance().getConfigPath().toString()+File.separator + "level.dat"), location);
			} catch (IOException e) {
